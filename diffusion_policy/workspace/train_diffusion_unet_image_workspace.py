...
import numpy as np
import shutil
from diffusion_policy.workspace.base_workspace import BaseWorkspace #基本的模型配置
from diffusion_policy.policy.diffusion_unet_image_policy import DiffusionUnetImagePolicy #Unet
from diffusion_policy.dataset.base_dataset import BaseImageDataset, BaseDataset #数据集
from diffusion_policy.env_runner.base_image_runner import BaseImageRunner 
from diffusion_policy.common.checkpoint_util import TopKCheckpointManager
from diffusion_policy.common.json_logger import JsonLogger
from diffusion_policy.common.pytorch_util import dict_apply, optimizer_to
from diffusion_policy.model.diffusion.ema_model import EMAModel #ENA模型
from diffusion_policy.model.common.lr_scheduler import get_scheduler #学习率规划器
...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model模型配置，DiffusionUnet、视觉编码器、优化器
        ...
        # do not save optimizer if resume=False不保存优化器如果是resume模式
    ...
    def run(self):
        ...
        # configure dataset配置训练数据集
        dataset: BaseImageDataset
        dataset = hydra.utils.instantiate(cfg.task.dataset)
        assert isinstance(dataset, BaseImageDataset) or isinstance(dataset, BaseDataset)
        train_dataloader = DataLoader(dataset, **cfg.dataloader)

        # compute normalizer on the main process and save to disk在主进程上计算归一化器并保存到磁盘
        normalizer_path = os.path.join(self.output_dir, 'normalizer.pkl')
        if accelerator.is_main_process:
            normalizer = dataset.get_normalizer()
            pickle.dump(normalizer, open(normalizer_path, 'wb'))

        # load normalizer on all processes在所有进程上加载归一化器
        accelerator.wait_for_everyone()
        normalizer = pickle.load(open(normalizer_path, 'rb'))

        # configure validation dataset配置验证数据集
        ...
        # configure checkpoint配置checkpoint路径
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        # always use the latest batch总是使用最新的一批
                        ...
                        """
                        在每个小批次（mini-batch）上计算梯度，并在累积一定数量的小批次后进行一次梯度下降。这种方法称为梯度累积（gradient accumulation)
                        """
                        # step optimizer 每当累积一定数量的小批次（gradient_accumulate_every）后，执行一次梯度下降：
...
"""
使用@hydra.main装饰器定义了一个主函数main(cfg)。
这个装饰器会将Hydra配置系统应用于main函数。
配置路径设置为config目录，配置文件名为当前脚本文件的名字（不带扩展名）。
在main函数中，根据配置实例化TrainDiffusionUnetImageWorkspace对象并运行workspace.run()。
"""
"""
str(pathlib.Path(__file__).parent.parent.joinpath("config")):
    parent.parent属性：
        pathlib.Path(__file__): "/home/user/project/script/script.py"
        parent.parent.: "/home/user/project"
        joinpath("config"): "/home/user/project/config"

pathlib.Path(__file__).stem:
    Path(__file__):当前脚本的路径"/home/user/project/script/script.py"
    stem：获取文件名，不包括扩展名"script"
"""
"""
version_base:指定Hydra的版本基础设置。None表示不使用任何特定版本的基础配置
config_path：配置文件的路径
config_name：配置文件名
"""
@hydra.main(
