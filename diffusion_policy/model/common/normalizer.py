...
"""
这个类用于对数据进行归一化和反归一化
可以支持不同类型的输入数据（字典、张量、numpy、zarr数组）
归一化过程可以通过不同模式（limits和Gaussian）进行配置
若输入是字典，它可以处理多字段，并在params_dict中记录归一化参数。
"""
class LinearNormalizer(DictOfTensorMixin):
    avaliable_modes = ['limits', 'gaussian']
    
    """
    通过计算并存储每个字段(若数据是字典)或整个数据的归一化参数来拟合归一化器
    """
    @torch.no_grad()
    def fit(self,
    ...
    """
    获取指定键的SingleFieldLinearNormalizer
    """
    ...
    """
    归一化过程的实现，根据forward标志进行归一化或反归一化
    """
...
"""
类似LinearNormalizer，但只专注于单个字段
提供了拟合归一化参数的方法("fit"),
创建带有拟合参数的实例的方法（create_fit），
创建带有手动参数的实例的方法（create_manual），
以及创建身份归一化器的方法(create_identity)
"""
class SingleFieldLinearNormalizer(DictOfTensorMixin):
    ...
    """
    类方法：创建并拟合一个SingleFieldLinearNormalizer实例
    """
    @classmethod
    ...
    """
    类方法:创建一个带手动参数(scale\offset\input_stats_dict)的SingleFieldLinearNormalizer实例
    """
    @classmethod
    ...
    """
    @classmethod
    def create_identity(cls, dtype=torch.float32):#固定的fit参数
...
"""
计算所提供数据的归一化参数
根据指定模式("limit"，Gaussian)计算缩放和偏移量
将计算出的参数存储在一个nn.ParameterDict中。
"""
...
"""
根据提供的参数(params)对输入x进行归一化或者反归一化
调整x的形状以匹配缩放和偏移量的维度。
Forward = True为归一化，Fasle为反归一化
"""
