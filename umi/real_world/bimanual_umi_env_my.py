...
class BimanualUmiEnv:
    def __init__(self, 
            ...
            mirror_swap=False,
            # this latency compensates receive_timestamp这个延迟补偿了receive_timestamp
            # all in seconds一切都在几秒钟之内
            camera_obs_latency=0.125,
            # all in steps (relative to frequency)所有步骤(相对于频率)
        ...
        # compute resolution for vis为vis计算分辨率
        ...
        # HACK: Separate video setting for each camera#为每个摄像机单独设置视频
        # Elagto Cam Link 4k records at 4k 30fps #Elagto Cam Link 4k记录4k 30fps
        # Other capture card records at 720p 60fps#其他捕获卡记录720p 60fps
        resolution = list()
        capture_fps = list()
        ...
        for path in v4l_paths:
            if 'Cam_Link_4K' in path:#Cam_Link_4K相机
                res = (3840, 2160)
                fps = 30
                buf = 3
                bit_rate = 6000*1000
                ...
                def tf(data, input_res=res):
                    ...
                    data['color'] = img
                    return data
                transform.append(tf)#图像数据

            resolution.append(res)#分辨率
            capture_fps.append(fps)#采集帧频
            cap_buffer_size.append(buf)#相机的buffer size
            video_recorder.append(VideoRecorder.create_hevc_nvenc(#视频记录器
                fps=fps,
                input_pix_fmt='bgr24',
                bit_rate=bit_rate
            ))
            ...
            def vis_tf(data, input_res=res):
                ...
                data['color'] = img
                return data
            vis_transform.append(vis_tf)#视频数据
        ...
        camera = MultiUvcCamera(
            ...
            # send every frame immediately after arrival到达后立即发送每帧
            # ignores put_fps忽略了put_fps
        ...
        for rc in robots_config:#机器人配置，两个机器人就配置两次RTDEInterpolationController
        ...
        # for gc in grippers_config:#配置机器手控制
        ...
        self.camera = camera#相机
        
        self.robots = robots#机器人
        self.robots_config = robots_config#机器人配置信息
        self.grippers = grippers#手爪
        self.grippers_config = grippers_config#手爪配置信息

        self.multi_cam_vis = multi_cam_vis#多个相机可视化
        self.frequency = frequency
        self.max_obs_buffer_size = max_obs_buffer_size
        self.max_pos_speed = max_pos_speed
        self.max_rot_speed = max_rot_speed
        # timing时间设置
        self.camera_obs_latency = camera_obs_latency#相机延时
        ...
        # temp memory buffers临时缓冲区
        self.last_camera_data = None
        # recording buffers记录缓冲
    ...
    # ========= async env API ===========异步环境API
    def get_obs(self) -> dict:
        """
        Timestamp alignment policy时间戳对齐策略
        We assume the cameras used for obs are always [0, k - 1], where k is the number of robots我们假设用于obs的摄像机总是[0,k - 1]，其中k是机器人的数量
        All other cameras, find corresponding frame with the nearest timestamp所有其他摄像头，找到时间戳最近的对应帧
        All low-dim observations, interpolate with respect to 'current' time所有的低亮度观测，都是根据“当前”时间进行插值的
        """
        ...
        # get data
        # 60 Hz, camera_calibrated_timestamp相机匹配时间戳60HZ
        k = math.ceil(
            self.camera_obs_horizon * self.camera_down_sample_steps \
            * (60 / self.frequency)) + 2 # here 2 is adjustable, typically 1 should be enough这里2是可调的，通常1就足够了
        ...
        # both have more than n_obs_steps data两者都有超过n_obs_steps的数据
        last_robots_data = list()
        last_grippers_data = list()
        # 125/500 hz, robot_receive_timestamp机器人接收时间戳
        for robot in self.robots:
            last_robots_data.append(robot.get_all_state())
            #print("last_robots_data: !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!",last_robots_data)
        # 30 hz, gripper_receive_timestamp手爪接收时间戳
        # for gripper in self.grippers:
        #     last_grippers_data.append(gripper.get_all_state())

        # select align_camera_idx#选择匹配的相机id
        ...
        # align camera obs timestamps#匹配相机观测时间戳
        ...
        # obs_data to return (it only includes camera data at this stage)在这个阶段，它只包括相机数据
        obs_data = dict(camera_obs)

        # include camera timesteps包含相机的时间步
        obs_data['timestamp'] = camera_obs_timestamps

        # align robot obs匹配相机和观测
        ...
        gripper_obs = {
                f'robot0_gripper_width': np.array([[0.08],[0.08]]),
                #f'robot1_gripper_width': np.array([1,1,1,1,1,1])
            }
