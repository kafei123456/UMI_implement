...
class SharedNDArray(Generic[SharedT]):
    """Class to keep track of and retrieve the data in a shared array#类来跟踪和检索共享数组中的数据
    Attributes
    ----------
    shm
    ...
    def get(self) -> npt.NDArray[SharedT]:#获取共享内存的numpy数组值
        """Get a numpy array with access to the shared memory获取进程共享内存的值"""
        return np.ndarray(self.shape, dtype=self.dtype, buffer=self.shm.buf)

    def __del__(self):#关闭进程
        #在本例中，__del__ 方法用于在对象被销毁时关闭与共享内存相关联的资源，即释放共享内存。
        #这样做是为了确保在不再需要 SharedNDArray 对象时，相关的共享内存资源也能够得到正确地释放，从而避免内存泄漏和资源泄漏的问题。
        self.shm.close()
