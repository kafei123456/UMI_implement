...
def pose_distance(start_pose,end_pose):
    ...
    start_rot = st.Rotation.form_rotvec(start_pose[3:])
    end_rot = sr.Rotation.from_rotvec(end_pose[3:])
...
class PoseTrajectoryInterpolator:
    def __init__(self, times: np.ndarray, poses: np.ndarray):
        ...
        if len(time) == 1:
            ...
            self.pos_interp = si.interpld(times, rot, axis = 0, assume_sorted = True)
            self.rot_interp = si.Slerp(times, rot)
    @property
    def times(self)-> np.ndarray:#times = PoseTrajectoryInterpolator.times  # 通过属性方式调用
        if self.single_step:
            return self._times
        else:
            return self.pos_interp.xrange#时间范围[start, end]
    @property
    def poses(self) -> np.ndarray:
        if self.single_step:
            ...
            poses[:,3:] = self.rot_interps(self.times).as_rotvec()
            return poses
    ...
    def trim(self, start_t:float, end_t: float) -> "PoseTrajectoryInterpolator":
        ...
        all_times = np.unique(all_times)#删除重复项
    ...
    def drive_to_waypoint(self, pose, time, curr_time, max_pos_speed = np.inf, max_rot_speed = np.inf) -> "PoseTrajectoryInterpolator":
        ...
        poses = np.append(trimmed_interp.pose, [pose], axis = 0)
        
        final_interp = PoseTrajectoryInterpolator(times, poses)
        return final_interp
    
    def schedule_waypoint(self, pose, time, 
                max_pos_speed = np.inf, 
                max_rot_speed = np.inf, 
                curr_time = None, 
                last_waypoint_time = None) -> "PoseTrajectoryInterpolator:"
        assert(max_pos_speed > 0)
        assert(max_rot_speed > 0)
        ...
        # end time应该是除time之外所有时间中最近的
        # 在此之后，我们可以假设顺序(由zhenjia证明，由于2分钟的操作)
        ...
        # time can't change不能改变time
        # last_waypoint_time can't change不能改变last_waypoint_time
        # curr_time can't change 不能改变curr_time
        assert start_time <= end_time
        ...
        #在此之后，trimmed_interp中的所有路点都在start_time和end_time之内
        # 而且比time要更早
